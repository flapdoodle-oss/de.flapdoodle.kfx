...
class VirtualView<T: Node>(
  private val content: T,
  boundsInParentProperty: (T) -> ObservableValue<javafx.geometry.Bounds>,
  subscribeToViewBounds: (ObjectBinding<javafx.geometry.Bounds>) -> Unit = { },
  ...
  private val nodeBoundingBoxProperty = boundsInParentProperty(content)
  private val zoomedBounds = nodeBoundingBoxProperty.and(zoom.mapToDouble()).map(BoundingBoxes::multiply)
  private val scrollXBounds = widthProperty().and(zoomedBounds).and(content.layoutXProperty()).map(ScrollBounds.Companion::widthOf)
  private val scrollYBounds = heightProperty().and(zoomedBounds).and(content.layoutYProperty()).map(ScrollBounds.Companion::heightOf)
  
  private val viewBounds: ObjectBindings.Merge3<javafx.geometry.Bounds, Number, Number, javafx.geometry.Bounds> = layoutBoundsProperty()
    .and(insetsProperty())
    .map { bounds, insets -> bounds - insets }
    .and(content.layoutXProperty())
    .and(content.layoutYProperty())
    .map { b, _, _ -> content.parentToLocal(b) }
...
//  }
  ...
  init {
    ...
    stylesheets += javaClass.getResource("VirtualView.css").toExternalForm()

//    layers.addHints(nodeConnectionHint)

    content.transforms.add(Scale().apply {
      xProperty().bind(zoom)
      yProperty().bind(zoom)
    })
//    content.visibleArea().bind(viewBounds)
    subscribeToViewBounds(viewBounds)

    children.add(content)

    scrollX.orientation = Orientation.HORIZONTAL
    scrollX.valueProperty().bindBidirectional(content.layoutXProperty())
    ...
    scrollY.valueProperty().bindBidirectional(content.layoutYProperty())
      ...
      MouseEvent.MOUSE_PRESSED -> {
        sharedEventLock.tryLock(this) {
          ...
          Action.Pan(
            ...
            posAtClick = content.layoutPosition
          )
        }
      }
  ...
  private fun handleScroll(pEvent: ScrollEvent) {
    ...
    if (!pEvent.isDirect && pEvent.touchCount <= 0) {
      ...
      if (pEvent.isControlDown) {
        val local = content.screenToLocal(Point2D(pEvent.screenX, pEvent.screenY))
//        println("zoom at ${pEvent.x},${pEvent.y} -> (screen: ${pEvent.screenX}, ${pEvent.screenY}) local: ${local.x},${local.y}) on ${wrapper.boundsInParent}")
        ...
        sharedEventLock.ifUnlocked {
          panTo(content.layoutX + pEvent.deltaX, content.layoutY + pEvent.deltaY)
          pEvent.consume()
        }
      }
    }
  ...
  fun setZoom(pZoom: Double) {
    setZoomAt(pZoom, content.layoutX, content.layoutY)
  }
  ...
  fun setZoomAt(pZoom: Double, pPivotX: Double, pPivotY: Double) {
    ...
    if (newZoomLevel != oldZoomLevel) {
      ...
      val pivotInWrapperSpace = content.parentToLocal(Point2D(pPivotX, pPivotY))
...
//      println("$pivotInWrapperSpace -> ${wrapper.layoutX},${wrapper.layoutY}")
      panTo(content.layoutX + diffX, content.layoutY + diffY)
    }
  }
